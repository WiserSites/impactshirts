...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_checkbox_list( $field, $meta ) {
    ...
    $this->show_field_begin($field, $meta);
      ...
      foreach ($field['options'] as $key => $value) {
        $html[] = "<div class='checkboxItem'><input type='checkbox' ".( isset($field['style'])? "style='{$field['style']}' " : '' )."  class='at-checkbox_list".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}[]' value='{$key}'" . checked( in_array( $key, $meta ), true, false ) . " /> {$value} </div>";
      }
