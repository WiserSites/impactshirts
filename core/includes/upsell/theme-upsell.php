 ...
 * Title: Theme Upsell.
 *
 * Description: Displays list of all WiserSites theme linking to it's pro and free versions.
 *
 * Please do not edit this file. This file is part of the WiserSites Framework and all modifications
 * should be made in a child theme.
 *
 * @category WiserSites Framework
 * @package  Framework
 * @since    1.0
 * @author   WiserSites
 * @license  http://www.opensource.org/licenses/gpl-license.php GPL v3.0 (or later)
 * @link     http://www.wisersites.com/
 */

// Add stylesheet and JS for upsell page.
function wisersites_upsell_style() {
	...
	wp_enqueue_style( 'wisersites-impactshirts', $directory_uri . '/core/includes/upsell/bootstrap/css/wisersites-impactshirts' . $suffix . '.css', array( 'bootstrap', 'bootstrap-impactshirts' ) );
...
function wisersites_add_upsell() {
	$page = add_theme_page(
		'More Themes',
		'More Themes',
		'administrator',
		'wisersites-themes',
		'wisersites_display_upsell'
	);

	add_action( 'admin_print_styles-' . $page, 'wisersites_upsell_style' );
}

add_action( 'admin_menu', 'wisersites_add_upsell' );

// Define markup for the upsell page.
function wisersites_display_upsell() {
			...
			<div id="upsell_container">
				<div class="row-fluid">
					<div id="upsell_header" class="span12">
						<h2>
							<a href="http://wisersites.com" target="_blank">
								<img src="<?php echo $directory_uri; ?>/core/includes/upsell/images/upsell-logo.png"/>
							</a>
						</h2>

						<h3><?php _e( 'Themes You Can Trust', 'wisersites_core' ); ?></h3>
					</div>
				</div>
				...
				<div id="upsell_themes" class="row-fluid">
					...
					$themes = wisersites_get_themes( $request );
					...
					foreach ( $themes->themes as $theme ) {
						if ( $active_theme != $theme->name ) {
							...
							$theme_details = wisersites_get_themes( $request );
							?>
...
// Get all WiserSites themes by using API.
function wisersites_get_themes( $request ) {

	// Generate a cache key that would hold the response for this request:
	$key = 'wisersites_' . md5( serialize( $request ) );
