...
$imageLink		= get_tax_meta($term->term_id,'categoryImageLink');
...
<div id="content-full" class="grid col-940"><!-- taxonomy.php -->
	...
	if($headerTitle && $smallTitle && $description):
		...
		if (function_exists('socialWarfare')) :
			...
			echo '<h1 style="text-align:left;">'. $headerTitle .'</h1>';
			echo '</div>';
		...
		echo '</div>';
		echo '<div class="grid col-220 fit garmentLiveText">';
	...
	<?php 
		global $query_string;
		query_posts( $query_string . '&posts_per_page='.$data['pagiDesigns'] );
		if( have_posts() ) : 
