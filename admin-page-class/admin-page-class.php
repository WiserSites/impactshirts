 ...
 */
  ...
  */
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
         foreach ($meta as $me){
           ...
           echo '<div class="at-repater-block"><img src="';
        if ($this->_Local_images){
          echo $plugin_path.'/images/remove.png';
        }else{
          echo 'http://i.imgur.com/g8Duj.png';
        }
        echo '" alt="'.__('Remove','apc').'" title="'.__('Remove','apc').'" id="remove-'.$field['id'].'"><table class="repeater-table">';
           if ($field['inline']){
    ...
    echo '" alt="'.__('Remove','apc').'" title="'.__('Remove','apc').'" id="remove-'.$jsid.'"><table class="repeater-table">';
    if ($field['inline']){
      echo '<tr class="at-inline" VALIGN="top">';
    } 
    foreach ($field['fields'] as $f){
...
.at-repater-block{background-color: #FFFFFF;margin: 2px;}
</style>';
  ...
  public function show_field_sortable( $field, $meta ) {
    ...
    $re .= '</ul><div class="clearfix"></div></div>';
    echo $re;
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_image( $field, $meta ) {
    ...
    if (is_array($meta) && isset($meta['src']) && $meta['src'] != ''){
      $html .= "<div class='clearfix'></div><span class='mupload_img_holder' data-wi='".$width."' data-he='".$height."'><img src='".$meta['src']."' style='height: ".$height.";width: ".$width.";' /></span>";
      $html .= "<input type='hidden' name='".$field['id']."[id]' id='".$field['id']."[id]' value='".$meta['id']."' />";
      $html .= "<input type='hidden' name='".$field['id']."[src]' id='".$field['id']."[src]' value='".$meta['src']."' />";
      $html .= "<input class='at-delete_image_button button' type='button' data-u='".$upload_type."' rel='".$field['id']."' value='".__('Delete Image','apc')."' style='float:right' />";
    }else{
