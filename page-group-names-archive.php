...
/**
Template Name: Youth Group Names Archive
 */
	...
	// asort($fields['names']);
	if( count( $fields['group_names'] ) ){
		foreach($fields['group_names'] as $group_names){
			$single_fields = get_fields( $group_names['group_post_id'] );
			// Begin the loop
			$i = 0;
			foreach($single_fields['names'] as $group_name):
				$i++;
