...
*********************************************/
	...
	if (function_exists('socialWarfare') && $data['hide_title'] != true) :
	...
	foreach ( $data['modules'] as $module ):
		...
		*********************************************/
		...
		elseif($module['moduleType'] == 'groupNames'):
			...
			// Begin the gallery container
			echo '<div class="cat-gallery group-names cat_group_names">';
			echo '<ul>';
			$post_counter = 0;
			...
			if($module['group_names_viewAll']):
				...
				if( $link )
					echo '<a class="viewAllDesigns" href="'.$link.'">View All Group Names</a>';
			endif;

			echo '</div>';

			// Reset the WP Query so that it doesn't interfere with loops that may appear in other modules
			//wp_reset_query(); //not needed for my script )
		endif;
	endforeach;
